...
def shotGen_validate_epoch_MuLMINet(data_loader, encoder, decoder, criterion, encoder_optimizer, decoder_optimizer, record_loss, config, device="cpu"):
    ...
    for loader_idx, item in enumerate(data_loader):
        ...
        loss_height = criterion['entropy'](output_height_logits, target_height.long())
    ...
    return total_loss
def shotGen_train_epoch_MuLMINet(data_loader, encoder, decoder, criterion, encoder_optimizer, decoder_optimizer, config, record_loss, encoder_scheduler=None, decoder_scheduler=None, device="cpu"):
    encode_length = config['encode_length'] - 1         # use the first 3 strokes to the encoder
    ...
    for loader_idx, item in enumerate(data_loader):
        ...
        # This function is to replace the overflow value to numerical value.
        def replace_value(lst):
            for i in range(len(lst)):
                if lst[i] == -2147483648:
                    lst[i] = 1
    ...
    return total_loss
def shotgen_generator_MuLMINet(given_seq, encoder, decoder, config, samples, device):

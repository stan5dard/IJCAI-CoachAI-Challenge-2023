def shotGen_validate_epoch(data_loader, encoder, decoder, criterion, encoder_optimizer, decoder_optimizer, record_loss,
                           config, device="cpu"):
    ...
    for loader_idx, item in enumerate(data_loader):
        batch_input_shot_type, batch_input_landing_x, batch_input_landing_y, batch_input_landing_height = item[0].to(
            device), item[1].to(device), item[2].to(device), item[3].to(device)
        batch_input_aroundhead, batch_input_backhand, batch_input_player = item[4].to(device), item[5].to(device), item[
            6].to(device),
        batch_input_player_location_area, batch_input_opponent_location_area = item[7].to(device), item[8].to(device)
        batch_target_shot_type, batch_target_landing_x, batch_target_landing_y, batch_target_landing_height = item[
                                                                                                                  9].to(
            device), item[10].to(device), item[11].to(device), item[12].to(device)
        batch_target_aroundhead, batch_target_backhand, batch_target_player = item[13].to(device), item[14].to(device), \
                                                                              item[15].to(device)
        batch_target_player_location_area, batch_target_opponent_location_area = item[16].to(device), item[17].to(
            device)
        ...
        output_xy, output_shot_logits, output_height_logits, output_aroundhead_logits, output_backhand_logits, output_playerloc_logits, output_opponentloc_logits = decoder(
        ...
        loss = config['alpha'] * (loss_shot + loss_area) + (1 - config['alpha']) * (
                    loss_height + loss_aroundhead + loss_backhand + loss_playerloc + loss_opponentloc)
        # loss.backward()
...
def shotGen_validate_epoch_model_hybrid(data_loader, encoder, decoder, criterion, encoder_optimizer, decoder_optimizer,
                                        record_loss, config, device="cpu"):
    encode_length = config['encode_length'] - 1  # use the first 3 strokes to the encoder
    # print(encode_length)
    ...
    for loader_idx, item in enumerate(data_loader):
        ...
        # print('input_player : ', input_player[0].shape)
        encoder_result = encoder(input_shot, input_x, input_y, input_player, input_height, input_aroundhead,
...
def shotGen_validate_epoch_model3_small(data_loader, encoder, decoder, criterion, encoder_optimizer, decoder_optimizer,
                                        record_loss, config, device="cpu"):
    encode_length = config['encode_length'] - 1  # use the first 3 strokes to the encoder
    # print(encode_length)
    ...
    for loader_idx, item in enumerate(data_loader):
        ...
        # print('input_player : ', input_player[0].shape)
        encode_local_output_area, encode_local_output_shot, encode_global_A_area, encode_global_A_shot, encode_global_B_area, encode_global_B_shot = encoder(
            input_shot, input_x, input_y, input_player, input_aroundhead, input_backhand)
        # encode_local_output, encode_global_A, encode_global_B = encoder(input_shot, input_x, input_y, input_player, input_height, input_aroundhead, input_backhand, input_player_location_area, input_opponent_location_area)
        ...
        # print('target player : ', target_player[0].shape)
        output_xy, output_shot_logits, output_aroundhead_logits, output_backhand_logits = decoder(input_shot, input_x,
                                                                                                  input_y, input_player,
                                                                                                  input_aroundhead,
                                                                                                  input_backhand,
                                                                                                  encode_local_output_area,
                                                                                                  encode_local_output_shot,
                                                                                                  encode_global_A_area,
                                                                                                  encode_global_A_shot,
                                                                                                  encode_global_B_area,
                                                                                                  encode_global_B_shot,
                                                                                                  target_player)
...
def shotGen_validate_epoch_simple(data_loader, encoder, decoder, criterion, encoder_optimizer, decoder_optimizer,
                                  record_loss, config, device="cpu"):
    encode_length = config['encode_length'] - 1  # use the first 3 strokes to the encoder
    # print(encode_length)
    ...
    for loader_idx, item in enumerate(data_loader):
        ...
        # print('input_player : ', input_player[0].shape)
        encode_local_output_area, encode_local_output_shot, encode_global_A_area, encode_global_A_shot, encode_global_B_area, encode_global_B_shot = encoder(
            input_shot, input_x, input_y, input_player)
        # encode_local_output, encode_global_A, encode_global_B = encoder(input_shot, input_x, input_y, input_player, input_height, input_aroundhead, input_backhand, input_player_location_area, input_opponent_location_area)
        ...
        # print('target player : ', target_player[0].shape)
        output_xy, output_shot_logits = decoder(input_shot, input_x, input_y, input_player, encode_local_output_area,
                                                encode_local_output_shot, encode_global_A_area, encode_global_A_shot,
                                                encode_global_B_area, encode_global_B_shot, target_player)
...
def shotGen_validate_epoch_Big(data_loader, encoder, decoder, criterion, encoder_optimizer, decoder_optimizer,
                               record_loss, config, device="cpu"):
    encode_length = config['encode_length'] - 1  # use the first 3 strokes to the encoder
    # print(encode_length)
    ...
    for loader_idx, item in enumerate(data_loader):
        ...
        output_xy, output_shot_logits, output_height_logits, output_aroundhead_logits, output_backhand_logits, output_playerloc_logits, output_opponentloc_logits = decoder(
            ...
            encode_output_pan_A, encode_output_oan_A, encode_output_area_B, encode_output_shot_B, encode_output_bh_B,
            encode_output_ba_B, encode_output_bb_B, encode_output_pan_B, encode_output_oan_B, target_player)
...
def shotGen_validate_epoch_Big2(data_loader, encoder, decoder, criterion, encoder_optimizer, decoder_optimizer,
                                record_loss, config, device="cpu"):
    encode_length = config['encode_length'] - 1  # use the first 3 strokes to the encoder
    # print(encode_length)
    ...
    for loader_idx, item in enumerate(data_loader):
        ...
        # print('target player : ', target_player[0].shape)
        output_xy, output_shot_logits, output_height_logits, output_aroundhead_logits, output_backhand_logits, output_playerloc_logits, output_opponentloc_logits, output_pl_xy, output_ol_xy = decoder(
            input_shot, input_x, input_y, input_player, input_height, input_aroundhead, input_backhand,
            input_player_location_area, input_opponent_location_area, input_player_location_x, input_player_location_y,
            input_opponent_location_x, input_opponent_location_y,
            ...
            encode_output_area_A, encode_output_shot_A, encode_output_bh_A, encode_output_ba_A, encode_output_bb_A,
            encode_output_pan_A, encode_output_oan_A, encode_output_pl_A, encode_output_pl_B,
            encode_output_area_B, encode_output_shot_B, encode_output_bh_B, encode_output_ba_B, encode_output_bb_B,
            encode_output_pan_B, encode_output_oan_B, encode_output_pl_B, encode_output_ol_B, target_player)
        ...
        gold_pl_xy = torch.cat((target_pl_x.unsqueeze(-1), target_pl_y.unsqueeze(-1)), dim=-1).to(device,
                                                                                                  dtype=torch.float)
        loss_playerxy = Gaussian2D_loss(output_pl_xy, gold_pl_xy)

        gold_ol_xy = torch.cat((target_ol_x.unsqueeze(-1), target_ol_y.unsqueeze(-1)), dim=-1).to(device,
                                                                                                  dtype=torch.float)
        ...
        loss = config['alpha'] * (loss_shot + loss_area) + (1 - config['alpha']) * (
                    loss_height + loss_aroundhead + loss_backhand + loss_playerloc + loss_opponentloc + loss_playerxy + loss_opponentxy)
        # loss.backward()
...
def shotGen_train_epoch_simple(data_loader, encoder, decoder, criterion, encoder_optimizer, decoder_optimizer, config,
                               record_loss, device="cpu"):
    encode_length = config['encode_length'] - 1  # use the first 3 strokes to the encoder
    ...
    for loader_idx, item in enumerate(data_loader):
        ...
        output_xy, output_shot_logits = decoder(input_shot, input_x, input_y, input_player, encode_local_output_area,
                                                encode_local_output_shot,
                                                encode_global_A_area, encode_global_A_shot, encode_global_B_area,
                                                encode_global_B_shot, target_player)
...
def shotGen_train_epoch_Big(data_loader, encoder, decoder, criterion, encoder_optimizer, decoder_optimizer, config,
                            record_loss, encoder_scheduler=None, decoder_scheduler=None, device="cpu"):
    encode_length = config['encode_length'] - 1  # use the first 3 strokes to the encoder
...
def shotGen_train_epoch_Big2(data_loader, encoder, decoder, criterion, encoder_optimizer, decoder_optimizer, config,
                             record_loss, encoder_scheduler=None, decoder_scheduler=None, device="cpu"):
    encode_length = config['encode_length'] - 1  # use the first 3 strokes to the encoder
    # print(encode_length)
...
def shotGen_train_epoch_model3_small(data_loader, encoder, decoder, criterion, encoder_optimizer, decoder_optimizer,
                                     record_loss, config, device="cpu"):
    encode_length = config['encode_length'] - 1  # use the first 3 strokes to the encoder
    # print(encode_length)
        ...
        for sample_id in range(samples):
            ...
            for seq_idx in range(encode_length, given_seq['rally_length'] - 1):
                ...
                if seq_idx == encode_length:
                    ...
                    input_aroundhead = given_seq['given_ball_aroundhead'][seq_idx].unsqueeze(0).unsqueeze(0).to(
                        torch.long)
    ...
    with torch.no_grad():
        ...
        for sample_id in range(samples):
            ...
            for seq_idx in range(encode_length, given_seq['rally_length'] - 1):
                ...
                output_xy, output_shot_logits = decoder(input_shot, input_x, input_y, input_player,
                                                        encode_local_output_area, encode_local_output_shot,
                                                        encode_global_A_area, encode_global_A_shot,
                                                        encode_global_B_area, encode_global_B_shot, target_player)
            ...
            for seq_idx in range(encode_length, given_seq['rally_length'] - 1):
                ...
                if seq_idx == encode_length:
                    ...
                    input_aroundhead = given_seq['given_ball_aroundhead'][seq_idx].unsqueeze(0).unsqueeze(0).to(
                        torch.long)
                    input_backhand = given_seq['given_ball_backhand'][seq_idx].unsqueeze(0).unsqueeze(0).to(torch.long)
                    input_player_location_area = given_seq['given_ball_player_area'][seq_idx].unsqueeze(0).unsqueeze(
                        0).to(torch.long)
                    input_opponent_location_area = given_seq['given_ball_opponent_area'][seq_idx].unsqueeze(
                    ...
                    input_aroundhead = torch.cat((input_aroundhead, prev_aroundhead), dim=-1)
                    input_backhand = torch.cat((input_backhand, prev_backhand), dim=-1)
                    input_player_location_area = torch.cat((input_player_location_area, prev_player_location_area),
                                                           dim=-1)
                    input_opponent_location_area = torch.cat(
                        (input_opponent_location_area, prev_opponent_location_area), dim=-1)
                ...
                output_xy, output_shot_logits, output_height_logits, output_aroundhead_logits, output_backhand_logits, output_playerloc_logits, output_opponentloc_logits = decoder(
                    ...
                    input_player_location_area, input_opponent_location_area, encode_output_area, encode_output_shot,
                    encode_output_bh, encode_output_ba, encode_output_bb, encode_output_pan, encode_output_oan,
                    encode_output_area_A, encode_output_shot_A, encode_output_bh_A, encode_output_ba_A,
                    encode_output_bb_A, encode_output_pan_A, encode_output_oan_A, encode_output_area_B,
                    encode_output_shot_B, encode_output_bh_B, encode_output_ba_B, encode_output_bb_B,
                    encode_output_pan_B, encode_output_oan_B, target_player)
        ...
        for sample_id in range(samples):
            ...
            for seq_idx in range(encode_length, given_seq['rally_length'] - 1):
                ...
                if seq_idx == encode_length:
                    ...
                    input_opponent_location_x = given_seq['given_opponent_location_x'][seq_idx].unsqueeze(0).unsqueeze(
                        0)
                    input_opponent_location_y = given_seq['given_opponent_location_y'][seq_idx].unsqueeze(0).unsqueeze(
                        0)
                ...
                output_xy, output_shot_logits, output_height_logits, output_aroundhead_logits, output_backhand_logits, output_playerloc_logits, output_opponentloc_logits, output_pl_xy, output_ol_xy = decoder(
                    ...
                    input_player_location_y,
                    encode_output_area, encode_output_shot, encode_output_bh, encode_output_ba, encode_output_bb,
                    encode_output_pan, encode_output_oan, encode_output_pl, encode_output_ol,
                    encode_output_area_A, encode_output_shot_A, encode_output_bh_A, encode_output_ba_A,
                    encode_output_bb_A, encode_output_pan_A, encode_output_oan_A, encode_output_pl_A,
                    encode_output_ol_A,
                    ...
                    encode_output_ol_B, target_player)
...
def shotGen_train_epoch(data_loader, encoder, decoder, criterion, encoder_optimizer, decoder_optimizer, config,
                        record_loss, encoder_scheduler=None, decoder_scheduler=None, device="cpu"):
    encode_length = config['encode_length'] - 1  # use the first 3 strokes to the encoder
...
def shotGen_validate_epoch_model3_crl(data_loader, encoder, decoder, criterion, encoder_optimizer, decoder_optimizer,
                                      record_loss, config, device="cpu"):
    encode_length = config['encode_length'] - 1  # use the first 3 strokes to the encoder
    # print(encode_length)
    ...
    for loader_idx, item in enumerate(data_loader):
        ...
        output_xy, output_shot_logits, output_height_logits, output_aroundhead_logits, output_backhand_logits, output_playerloc_logits, output_opponentloc_logits = decoder(
            ...
            encode_local_output_shot, encode_global_A_area, encode_global_A_shot, encode_global_B_area,
            encode_global_B_shot, target_player)
...
def shotGen_train_epoch_model3_crl(data_loader, encoder, decoder, criterion, encoder_optimizer, decoder_optimizer,
                                   config, record_loss, encoder_scheduler=None, decoder_scheduler=None, device="cpu"):
    encode_length = config['encode_length'] - 1  # use the first 3 strokes to the encoder
...
def shotGen_trainer(data_loader, encoder, decoder, criterion, encoder_optimizer, decoder_optimizer, config,
                    device="cpu"):
    encode_length = config['encode_length'] - 1  # use the first 3 strokes to the encoder
    record_loss = {
        'total': [],
    ...
    for epoch in tqdm(range(config['epochs']), desc='Epoch: '):
        ...
        for loader_idx, item in enumerate(data_loader):
            batch_input_shot_type, batch_input_landing_x, batch_input_landing_y, batch_input_landing_height = item[
                                                                                                                  0].to(
                device), item[1].to(device), item[2].to(device), item[3].to(device)
            batch_input_aroundhead, batch_input_backhand, batch_input_player = item[4].to(device), item[5].to(device), \
                                                                               item[6].to(device),
            batch_input_player_location_area, batch_input_opponent_location_area = item[7].to(device), item[8].to(
                device)
            batch_target_shot_type, batch_target_landing_x, batch_target_landing_y, batch_target_landing_height = item[
                                                                                                                      9].to(
                device), item[10].to(device), item[11].to(device), item[12].to(device)
            batch_target_aroundhead, batch_target_backhand, batch_target_player = item[13].to(device), item[14].to(
                device), item[15].to(device)
            batch_target_player_location_area, batch_target_opponent_location_area = item[16].to(device), item[17].to(
                device)
            seq_len, seq_sets = item[18].to(device), item[19].to(device)
            ...
            encode_local_output, encode_global_A, encode_global_B = encoder(input_shot, input_x, input_y, input_player,
                                                                            ...
                                                                            input_backhand, input_player_location_area,
                                                                            input_opponent_location_area)
            ...
            loss_shot = criterion['entropy'](output_shot_logits,
                                             target_shot.long())  # target_shot에서 target_shot.long()로 바꿈
            loss_area = Gaussian2D_loss(output_xy, gold_xy)

...
class StrokeEvaluator:
    ...
    def evaluate(self, path):
        if self.filename is not None:
            self.prediction = pd.read_csv(f"{path}/prediction{self.filename}.csv")
        else:
            self.prediction = pd.read_csv(f"{path}/predictiontest.csv")
        self.check_and_convert_type_prob_with_softmax()
        self.compute_metrics()
    ...
    def compute_metrics(self):
        ...
        if self.filename is not None:
            record_df.to_csv(f"{self.path}/record_score{self.filename}.csv")
        else:
            record_df.to_csv(f"{self.path}/record_score.csv")
        with open(f"{self.path}/err{rally_score[-1]}", 'w') as error_marker:
            error_marker.write(str(rally_score[-1]))

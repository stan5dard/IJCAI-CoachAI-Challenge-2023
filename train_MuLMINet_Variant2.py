            ...
            for dim_value in dim_list:
                for alpha in alpha_list:
                    for layer in layer_list:
                        ...
                        hyper = 'lr' + str(lr_value) + 'bat' + str(batch_value) + 'dim' + str(
                            dim_value) + 'alpha' + str(alpha) + 'layer' + str(layer) + 'epoch' + str(config['epochs'])
                        print('')
                        print(hyper)
                        ...
                        config, train_dataloader, test_dataloader, train_dataset, test_dataset, train_matches, test_matches = prepare_dataset_cross(
                            config)
                        ...
                        # read model
                        from MuLMINet.MuLMINet import ShotGenEncoder_MuLMINet_Variant2, \
                            ShotGenPredictor_MuLMINet_Variant2
                        from MuLMINet.MuLMINet_runner import shotGen_train_epoch_MuLMINet, \
                            shotGen_validate_epoch_MuLMINet
                        ...
                        for fold, (train_idx, val_idx) in enumerate(kfold.split(train_dataset)):
                            ...
                            trainloader = torch.utils.data.DataLoader(train_dataset, batch_size=config['batch_size'],
                                                                      sampler=train_subsampler)  # 해당하는 index 추출
                            valloader = torch.utils.data.DataLoader(train_dataset, batch_size=config['batch_size'],
                                                                    sampler=val_subsampler)

                            encoder = ShotGenEncoder_MuLMINet_Variant2(config)
                            decoder = ShotGenPredictor_MuLMINet_Variant2(config)
                            ...
                            for epoch in tqdm(range(config['epochs']), desc='Epoch: '):
                                new_train_loss = shotGen_train_epoch_MuLMINet(data_loader=trainloader, encoder=encoder,
                                                                              decoder=decoder, criterion=criterion,
                                                                              encoder_optimizer=encoder_optimizer,
                                                                              decoder_optimizer=decoder_optimizer,
                                                                              config=config,
                                                                              record_loss=record_train_loss,
                                                                              device=device)
                                new_val_loss = shotGen_validate_epoch_MuLMINet(data_loader=valloader, encoder=encoder,
                                                                               decoder=decoder, criterion=criterion,
                                                                               encoder_optimizer=encoder_optimizer,
                                                                               decoder_optimizer=decoder_optimizer,
                                                                               config=config,
                                                                               record_loss=record_validation_loss,
                                                                               device=device)

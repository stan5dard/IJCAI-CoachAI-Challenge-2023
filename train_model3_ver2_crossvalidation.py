...
if __name__ == "__main__":
    ...
    dim_list = [256]
    layer_list = [1]
    # alpha_list = [0.8, 0.7, 0.6, 0.5, 0.4, 0.3, 0.2]
    alpha_list = [0.45]
            ...
            for dim_value in dim_list:
                for alpha in alpha_list:
                    for layer in layer_list:
                        ...
                        # read model
                        from ShuttleNet.ShuttleNet import ShotGenEncoder_model3_ver2, ShotGenPredictor_model3_ver2, ShotGenEncoder, ShotGenPredictor
                        ...
                        for fold, (train_idx, val_idx) in enumerate(kfold.split(train_dataset)):
                            ...
                            for epoch in tqdm(range(config['epochs']), desc='Epoch: '):
                                ...
                                if new_val_loss < val_loss:
                                    ...
                                    save_fold(encoder, decoder, config, new_val_loss, fold)
                                EarlyStopping(new_val_loss)
                            draw_loss_cross(record_train_loss, record_validation_loss, config, fold)
                            generate(config['output_folder_name'], f"{fold}")
